...
class BasicBlock3D(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv2 = nn.Sequential(nn.ReLU(True), conv3x3x3(planes, planes))
...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_class=10):
        ...
        factor = 1
        self.in_planes = int(32 * factor)
        self.conv1 = conv3x3x3(1, int(32 * factor))
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        downsample = nn.Sequential()
        if stride != 1 or self.in_planes != planes * block.expansion:
            downsample = DownsampleB3D(self.in_planes, planes * block.expansion, 2)
...
def resnet3d(num_class=10, blocks=BasicBlock3D):
    return ResNet3D(blocks, [1, 1, 1], num_class)
